...
function valueClassName(key: string): string {
  if (key === "c") {
    return cn(
      ...
      "whitespace-nowrap text-[clamp(0.72rem,2.8vw,0.95rem)] sm:text-[13px]",
    );
  }
  if (key === "energy" || key === "diff") {
    return cn(
      "mt-0.5 w-full min-w-0 font-semibold tabular-nums leading-tight",
...
/** Kartu statistik compact satu baris — khusus modul Kalor. */
export function ThermalChangeQuickInfo({
  items,
}: {
    ...
    >
      {items.map(({ key, label, value, valueClassName: extra }) => (
        ...
        >
          ...
          {key === "c" ? (
            <p className="mt-0.5 text-[8px] leading-none text-muted-foreground sm:text-[9px]">
              J/kg°C
            </p>
          ) : null}
        </div>
