...
function valueClassName(key: string): string {
  if (key === "status") {
    return cn(
      "mt-0.5 w-full min-w-0 text-center font-semibold leading-[1.1]",
      "whitespace-normal text-[clamp(0.68rem,2.85vw,0.95rem)] sm:text-[15px]",
      "line-clamp-2",
    );
  }
  if (key === "n") {
    return cn(
      ...
      "whitespace-nowrap text-[clamp(0.72rem,3.05vw,1rem)] sm:text-[15px]",
    );
  }
  if (key === "hukum") {
    return cn(
  ...
  return cn(
    "mt-0.5 w-full min-w-0 font-semibold tabular-nums leading-tight",
    "whitespace-nowrap text-[clamp(0.9rem,3.7vw,1.15rem)] sm:text-[16px]",
  );
}

/**
 * Kartu statistik compact satu baris — khusus modul Pemantulan & Pembiasan Cahaya.
 */
export function LightOpticsQuickInfo({
  items,
}: {
  ...
  return (
    <div
      className={cn(
        ...
        "grid-cols-[minmax(0,0.85fr)_minmax(0,0.85fr)_minmax(0,1.05fr)_minmax(0,1.25fr)]",
        "max-[339px]:grid-cols-[repeat(4,minmax(72px,1fr))]",
        "sm:grid-cols-4",
      )}
    >
